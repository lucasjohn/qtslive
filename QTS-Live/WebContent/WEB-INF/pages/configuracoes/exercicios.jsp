...
<body>
	...
	<div class="panel panel-white">
		...
		<div class="panel-heading clearfix">
			<h4 class="panel-title">Exercício</h4>
		</div>
		<div class="panel-body">
			<button id="novo-registro-button" type="button" class="btn btn-primary btn-addon m-b-sm" data-toggle="modal" data-target="#modalNovoRegistro"> Novo Exercício </button>
	                ...
	                <div class="modal-dialog">
	                    <div class="modal-content">
	                        <div class="modal-header">
	                            ...
	                            <h4 class="modal-title" id="myModalLabel">Adicionar Exercício</h4>
	                        </div>
...
<!-- 	                        		<div class="col-sm-5"> -->
										<div class="form-group">
		                                    <textarea id="novo-descricao-textarea" class="form-control" placeholder="Descrição" required="" type="text" rows="7"></textarea>                                   
		                                </div>
<!-- 									</div> -->
	                        	</div>
	                        </div>
	            ...
	            <div style="display: none;" class="modal fade" id="editar-registro-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	                <div class="modal-dialog">
	                    <div class="modal-content">
	                        <div class="modal-header">
	                            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
	                            <h4 class="modal-title" id="myModalLabel">Alterar Exercício</h4>
	                        </div>
	                        <div class="modal-body">
                        		<div class="row">
<!-- 	                        		<div class="col-sm-5"> -->
...
<!-- 	                        		<div class="col-sm-5"> -->
										<div class="form-group">
		                                    <textarea id="descricao-textarea" class="form-control" placeholder="Descrição" required="" type="text" rows="7"></textarea>                                   
		                                </div>
<!-- 									</div> -->
	                        	</div> 
	                        </div>
            ...
            <div style="display: none;" class="modal fade" id="delete-registro-modal" tabindex="-1" role="dialog alert" aria-labelledby="myModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
                            <h4 class="modal-title" id="myModalLabel">Excluir Exercício</h4>
                        </div>
                        <div class="modal-body">
					...
					<table id="registros-table" class="display compact dataTable">
						...
						<tbody>
							<c:forEach items="${lista}" var="l" varStatus="status"> 
								<tr id="${l.id}">									
									<td title="${l.descricao}">${l.nome}</td>
									<td>
										<div class="btn-group m-b-sm" style="height: 6px">
                                            ...
                                            <ul class="dropdown-menu" role="menu">
                                                <li><a data-toggle="modal" data-target="#editar-registro-modal" href="#" onclick="iniciarEdicaoRegistro('${l.id}','${l.idClube}','${l.nome}','${l.descricao}')">Editar</a></li>
	...
	<content tag="local_script">
		...
		<script>
			...
			$(document).ready(function() {
				...
				$('#add-row').click(function () {
					...
					if($("#add-row-form").isValid()) {
			            ...
			            var	nome = $('#novo-nome-input').val(),
			                ...
			                descricao = $('#novo-descricao-textarea').val();
			            
			            
			            var params = "?exercicio.nome="+nome+"&exercicio.idClube="+idClube+"&exercicio.descricao="+descricao;
			            
			            //Envia a requisição
			            $.ajax({
			                type: "POST",
			                url: 'criarExercicioJson'+params,			                
			                dataType: 'json',
			                contentType: 'application/json',
			                success: function(data) {
			                	var id = data.exercicio.id;			                	
			                	var idClube = data.exercicio.idClube;
				                var nome = data.exercicio.nome;	
				                
				                //Adiciona a linha
			                	var rowIndex = $('#registros-table').dataTable().fnAddData([
												               				                nome,		               
												               				                gerarBotaoEditar(id, idClube, nome, descricao)
												               				              ]);
				...
				$('#update-row').click(function () {					
					...
					if($("#update-row-form").isValid()) {
			            ...
			            var params = "?exercicio.nome="+nome+"&exercicio.idClube="+idClube+"&exercicio.id="+id+"&exercicio.descricao="+descricao;
			            
			            $.ajax({
			                type: "POST",
			                url: 'editarExercicioJson'+params,			                
			                dataType: 'json',
			                contentType: 'application/json',
			                success: function(data) {
				...
				$('#delete-row').click(function () {					
			            ...
			            var params = "?exercicio.idClube="+idClube+"&exercicio.id="+id;
			            
			            $.ajax({
			                type: "POST",
			                url: 'excluirExercicioJson'+params,			                
			                dataType: 'json',
			                contentType: 'application/json',
			                success: function(data) {
			...
			function iniciarEdicaoRegistro(id, idClube, nome, descricao) {
				
				$('#id-input').val(id);				
	            $('#nome-input').val(nome);
	            $('#descricao-textarea').val(descricao);
			...
			function iniciarDeleteRegistro(id, idClube, nome) {	
				...
				$('#msg-alerta-excluir-div').html('<div class="alert alert-warning" role="alert">Excluir o exercício "<b>'+nome+'</b>"?</div>');
			...
			function gerarBotaoEditar(id, idClube, nome, descricao) {
